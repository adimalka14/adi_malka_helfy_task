...
function LabeledTextarea({ label, rows = 3, ...props }) {
    ...
    return (
        <label style={{ display: 'grid', gap: '0.25rem' }}>
            <span>{label}</span>
            <textarea {...field} rows={rows} {...props} />
            {meta.touched && meta.error && (
                <div style={{ color: '#ef4444', fontSize: '0.9rem' }}>{meta.error}</div>
            )}
        </label>
    ...
    return (
        <FormikProvider value={formik}>
            ...
            >
                ...
                <LabeledTextarea name="description" label="Description (optional)" placeholder="Details…" rows={3} />
                ...
                <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'space-between', marginTop: '0.4rem' }}>
                    {isEdit && onDelete && (
                        <button type="button" className="danger" onClick={() => onDelete(taskId)} disabled={formik.isSubmitting}>
                            Delete
                        </button>
                    )}
